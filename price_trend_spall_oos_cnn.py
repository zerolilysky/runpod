"""Standalone SP400/SP500/SP600-to-SPall out-of-sample I5 CNN.
...
The default run loads these files from ``data_dir``:
    ...
    SPall_picMap5MA_price_chart_image_df.pickle

SP400/SP500/SP600 are concatenated in that order and deduplicated by
``sedol + date``. The CNN is fit once on eligible pre-2020 observations and
then frozen for predictions on the deduplicated SPall observations from 2020.
...
price-trend script. This file has no dependency on another local price-trend
module and never reads a pre-existing predictions.csv as a model input.
"""
# ======================================================================
# price_trend_cnn.py
# ----------------------------------------------------------------------
...
import argparse, fnmatch, gc, glob, json, logging, time
...
def to_images(flat, H, W, order="C"):
    """Stack/reshape flattened chart vectors into an (N, H, W) cube.

    Binary/integer chart data stay uint8 in host memory. Conversion to float32
    happens per batch immediately before normalization and device transfer.
    ...
    return A.reshape(-1, H, W) if order == "C" else \
        np.stack([a.reshape(W, H).T for a in A]).astype(A.dtype, copy=False)
...
def train_cnn(Xtr, ytr, days, H, W, task="clf", seeds=5, epochs=50,
    ...
    if pixel_norm == "per_image":
        ...
        total = total_sq = 0.0
        count = 0
        stats_batch = max(8192, int(batch))
        for s0 in range(0, len(Xtr), stats_batch):
            chunk = np.asarray(Xtr[s0:s0 + stats_batch], dtype=np.float64)
            total += float(chunk.sum(dtype=np.float64))
            total_sq += float(np.square(chunk).sum(dtype=np.float64))
            count += int(chunk.size)
        mu = total / max(count, 1)
        variance = max(total_sq / max(count, 1) - mu * mu, 0.0)
        sd = float(np.sqrt(variance) + 1e-8)
        mu = float(mu)
        def _norm(A):
    ...
    non_blocking = str(dev).startswith("cuda")

    def _batch_tensors(indices):
        indices = np.asarray(indices, dtype=np.int64)
        images = _norm(Xtr[indices])
        xb = (torch.from_numpy(np.ascontiguousarray(images))
              .view(-1, 1, H, W).to(dev, non_blocking=non_blocking))
        labels = torch.from_numpy(np.ascontiguousarray(lab[indices]))
        yb = labels.to(dev, non_blocking=non_blocking)
        return xb, yb
    ...
    for k in range(seeds):
        ...
        shuffle_rng = np.random.default_rng(seed0 + k)
        for _ in range(epochs):
            net.train()
            order = shuffle_rng.permutation(len(ti))
            for s0 in range(0, len(ti), batch):
                b = ti[order[s0:s0 + batch]]
                if len(b) < 2:
                    continue
                xb, yb = _batch_tensors(b)
                opt.zero_grad()
            ...
            total, nobs = 0.0, 0
            with torch.no_grad():
                for s0 in range(0, len(vi), batch):
                    xb, yb = _batch_tensors(vi[s0:s0 + batch])
                    if len(yb) == 0:
                        continue
...
def _sp_source_priority(path):
    """Deterministic duplicate priority: SP400, then SP500, then SP600."""
    name = Path(path).name.lower()
    for priority, universe in enumerate(("sp400", "sp500", "sp600")):
        if universe in name:
            return priority, name
    return 3, name


def _normalize_sedol(values):
    """Normalize security IDs before grouping or duplicate detection."""
    return pd.Series(values, index=getattr(values, "index", None), dtype="string").str.strip().str.upper()
...
def load_panel(data_dir=None, pattern="R1000_picMap*_price_chart_image_df.pickle"):
    ...
    files = sorted(files, key=_sp_source_priority)
    ...
    df["sedol"] = _normalize_sedol(df["sedol"])
    df["date"] = pd.to_datetime(df["date"])
    df = df.dropna(subset=["price_chart_image"]).copy()
    rows_before_dedup = len(df)
    duplicate_rows_dropped = int(df.duplicated(["sedol", "date"]).sum())
    df = (df.drop_duplicates(["sedol", "date"], keep="first")
...
def _vix_coverage_issue(series, start=None, end=None, tolerance_days=7):
    ...
    if start is not None:
        ...
        if pd.Timestamp(series.index.min()) > requested_start + tolerance:
            return f"starts at {pd.Timestamp(series.index.min()).date()} after requested {requested_start.date()}"
    if end is not None:
        requested_end = pd.Timestamp(end)
        if pd.Timestamp(series.index.max()) < requested_end - tolerance:
            return f"ends at {pd.Timestamp(series.index.max()).date()} before requested {requested_end.date()}"
    return None
...
def load_vix(start=None, end=None, data_dir=None, symbol="^VIX",
    ...
    cached = None
    if fp.exists():
        cached = pd.read_csv(fp, parse_dates=["date"]).dropna().set_index("date")["vix"].sort_index()
        issue = _vix_coverage_issue(cached, start, end)
        if issue is None:
            return cached
        if not download:
            _hint(f"VIX cache at {fp} does not cover the requested dates ({issue})",
                  "extend vix.csv through the test period, or set download=True")
    elif not download:
    ...
    try:
        ...
        _hint("yfinance not installed and the VIX cache is missing or incomplete",
              "pip install yfinance, OR provide a vix.csv covering the requested dates")
    ...
    if isinstance(close, pd.DataFrame):          # newer yfinance MultiIndex cols
        close = close.iloc[:, 0]
    downloaded = pd.Series(np.asarray(close, float), index=pd.to_datetime(raw.index), name="vix")
    s = downloaded if cached is None else pd.concat([cached, downloaded])
    s = s[~s.index.duplicated(keep="last")]
    s.index.name = "date"
    s = s.sort_index()
    issue = _vix_coverage_issue(s, start, end)
    if issue is not None:
        _hint(f"downloaded VIX data does not cover the requested dates ({issue})",
              "check the symbol and requested date range")
...
class VixAugmenter:
    ...
    def strip_for_dates(self, dates):
        ...
        valid = j >= 0
        if valid.any():
            age = d[valid] - self.vdates[j[valid]]
            valid[np.flatnonzero(valid)] &= age <= np.timedelta64(7, "D")
        if not valid.all():
            raise RuntimeError(
                f"VIX is missing or more than 7 days stale for {int((~valid).sum())} image dates"
            )
...
def run(data_dir=None, pattern="R1000_picMap*_price_chart_image_df.pickle",
        test_pattern=None, test_data_dir=None,
    ...
    if cross:
        df.drop(columns=["price_chart_image"], inplace=True, errors="ignore")
        gc.collect()
        df_te, X_te = load_panel(test_data_dir or data_dir, test_pattern)
        ...
        df_te.drop(columns=["price_chart_image"], inplace=True, errors="ignore")
    else:
        df_te, X_te = df, X
        df.drop(columns=["price_chart_image"], inplace=True, errors="ignore")
    gc.collect()
    ...
    actual_train_date_min = None
    actual_train_date_max = None
    actual_test_date_min = None
    actual_test_date_max = None
    train_rows_used = 0
    ...
    if protocol == "faithful":
        ...
        train_dates_used = pd.to_datetime(df.loc[tr, "date"])
        test_dates_selected = pd.to_datetime(df_te.loc[te, "date"])
        actual_train_date_min = str(train_dates_used.min().date())
        actual_train_date_max = str(train_dates_used.max().date())
        actual_test_date_min = str(test_dates_selected.min().date())
        actual_test_date_max = str(test_dates_selected.max().date())
        train_rows_used = int(tr.sum())
        test_rows_selected = int(te.sum())
        ...
        Xtr_selected = np.ascontiguousarray(X[tr])
        Xte_selected = np.ascontiguousarray(X_te[te])
        X = None
        X_te = None
        gc.collect()
        predictor = _fit_predict(
            Xtr_selected, y[tr], df["date"].values[tr],
            Xte_selected, df_te["date"].values[te], preds, te,
    ...
    summary = {"tag": tag, "task": task, "protocol": protocol,
               ...
               "image_H": Hn, "image_W": W,
               "actual_train_date_min": actual_train_date_min,
               "actual_train_date_max": actual_train_date_max,
               "actual_test_date_min": actual_test_date_min,
               "actual_test_date_max": actual_test_date_max,
               "train_rows_used": train_rows_used,
               "test_rows_selected": test_rows_selected,
               "train_source_files": df.attrs.get("source_files", []),
               "test_source_files": df_te.attrs.get("source_files", []),
               "train_rows_before_dedup": df.attrs.get("rows_before_dedup"),
               "train_rows_after_dedup": df.attrs.get("rows_after_dedup"),
               "train_duplicate_rows_dropped": df.attrs.get("duplicate_rows_dropped"),
               "test_rows_before_dedup": df_te.attrs.get("rows_before_dedup"),
               "test_rows_after_dedup": df_te.attrs.get("rows_after_dedup"),
               "test_duplicate_rows_dropped": df_te.attrs.get("duplicate_rows_dropped"),
               **summ}
...
def _fit_predict(Xtr_raw, ytr, dtr, Xte_raw, dte, preds, te_mask,
    ...
    transformed = tf(Xtr_raw)
    Xtr = transformed if bool(np.all(keep)) else transformed[keep]
    ytr = ytr[keep]
...
# The embedded runner is captured below. The final SPall-aware CLI is invoked at
# the end of this standalone file.
...
def _find_source_pickle(data_dir, pattern):
    return _find_source_pickles(data_dir, pattern)[0]


def _find_source_pickles(data_dir, pattern):
...
def _write_filtered_training_input(
    ...
    if isinstance(source_pickle, (str, os.PathLike)):
        source_pickles = [Path(source_pickle)]
    else:
        source_pickles = [Path(path) for path in source_pickle]
    if not source_pickles:
        raise ValueError("at least one source pickle is required")
    source_pickles = sorted(source_pickles, key=_sp_source_priority)
    out_dir = Path(out_dir)
    target = _target_col(horizon)
    frames = [
        _coerce_to_df(pd.read_pickle(path), path.name).copy()
        for path in source_pickles
    ]
    df = pd.concat(frames, ignore_index=True)
    id_col = _ci(df.columns, ["sedol", "perm_ID", "permno", "id"])
    date_col = _ci(df.columns, ["date", "Date", "DATE"])
    image_col = _ci(df.columns, ["price_chart_image", "image", "img"])
    if not (id_col and date_col and image_col):
        raise KeyError("training inputs need an id, date, and price_chart_image column")
    df = df.rename(
        columns={id_col: "sedol", date_col: "date", image_col: "price_chart_image"}
    )
    df["sedol"] = _normalize_sedol(df["sedol"])
    if target not in df.columns:
        raise KeyError(
            f"target column {target!r} not found in "
            + ", ".join(path.name for path in source_pickles)
        )

    df["date"] = pd.to_datetime(df["date"])
    source_rows_before_dedup = len(df)
    source_duplicate_rows_dropped = int(df.duplicated(["sedol", "date"]).sum())
    df = df.drop_duplicates(["sedol", "date"], keep="first").reset_index(drop=True)
    train_cut = pd.Timestamp(train_cut)
    train_mask = df["date"] <= train_cut
    before = _label_counts(df, target, train_mask)
    suffixes = []
    meta = {
        "source_pickle": str(source_pickles[0]) if len(source_pickles) == 1 else None,
        "source_pickles": [str(path) for path in source_pickles],
        "source_pickle_count": len(source_pickles),
        "source_rows_before_dedup": int(source_rows_before_dedup),
        "source_rows_after_dedup": int(len(df)),
    ...
    if train_vix_lt is not None:
        ...
        if vix_series is None:
            vix_series = base.load_vix(
                ...
                data_dir=Path(data_dir) if data_dir else source_pickles[0].parent,
                symbol=vix_symbol,
                cache=vix_cache,
    ...
    source_stem = source_pickles[0].stem if len(source_pickles) == 1 else "combined_train"
    out_path = out_dir / f"{source_stem}_{'_'.join(suffixes)}.pickle"
...
def _align_series(series, dates, max_gap_days=7):
    ...
    if good.any():
        ...
        age = d[positions] - idx[loc[positions]]
        good[positions] &= age <= np.timedelta64(int(max_gap_days), "D")
...
SPALL_TRAIN_PATTERN = "SP[456]00_picMap5MA_price_chart_image_df.pickle"
SPALL_TEST_PATTERN = "SPall_picMap5MA_price_chart_image_df.pickle"
SPALL_TRAIN_FILENAMES = (
    "SP400_picMap5MA_price_chart_image_df.pickle",
    "SP500_picMap5MA_price_chart_image_df.pickle",
    "SP600_picMap5MA_price_chart_image_df.pickle",
)
...
def resolve_spall_inputs(data_dir=None, train_pattern=SPALL_TRAIN_PATTERN,
                         test_pattern=SPALL_TEST_PATTERN):
    """Resolve and validate the three training panels and the SPall test panel."""
    data_dir = Path(data_dir) if data_dir else Path.cwd()
    train_files = sorted(
        [Path(path).resolve() for path in base._find_files(data_dir, train_pattern)],
        key=_sp_source_priority,
    )
    test_files = [Path(path).resolve() for path in base._find_files(data_dir, test_pattern)]
    if not train_files:
        raise FileNotFoundError(
            f"no SP400/SP500/SP600 training files matching {train_pattern!r} in {data_dir}"
        )
    if not test_files:
        raise FileNotFoundError(
            f"no SPall test file matching {test_pattern!r} in {data_dir}"
        )

    if train_pattern == SPALL_TRAIN_PATTERN:
        found = {path.name.lower() for path in train_files}
        missing = [name for name in SPALL_TRAIN_FILENAMES if name.lower() not in found]
        if missing:
            raise FileNotFoundError(
                "missing required pre-2020 training file(s): " + ", ".join(missing)
            )
    if len(test_files) != 1:
        raise ValueError(
            f"expected exactly one SPall test file, found {len(test_files)}: "
            + ", ".join(path.name for path in test_files)
        )
    if set(train_files) & set(test_files):
        raise ValueError("a chart-image pickle cannot be both a training and test input")
    return {"train_files": train_files, "test_files": test_files}


def spall_input_manifest(data_dir=None, train_pattern=SPALL_TRAIN_PATTERN,
                         test_pattern=SPALL_TEST_PATTERN):
    """Return a lightweight input manifest without loading the large pickles."""
    resolved = resolve_spall_inputs(data_dir, train_pattern, test_pattern)
    rows = []
    for split, paths in resolved.items():
        split_name = "train" if split == "train_files" else "test"
...
def run(data_dir=None, pattern="sp00_5MA",
    ...
    if drop_train_abs_ret is not None or train_vix_lt is not None:
        ...
        source_pickles = _find_source_pickles(data_dir, pattern)
    ...
    if base_kwargs.get("test_pattern") is not None:
        base_kwargs.setdefault("test_data_dir", str(data_dir))
    ...
    audit_keys = (
        "actual_train_date_min",
        "actual_train_date_max",
        "actual_test_date_min",
        "actual_test_date_max",
        "train_rows_used",
        "test_rows_selected",
        "train_source_files",
        "test_source_files",
        "train_rows_before_dedup",
        "train_rows_after_dedup",
        "train_duplicate_rows_dropped",
        "test_rows_before_dedup",
        "test_rows_after_dedup",
        "test_duplicate_rows_dropped",
    )
    summary.update({key: source_summary[key] for key in audit_keys if key in source_summary})
...
def run_spall_oos(data_dir=None, train_pattern=SPALL_TRAIN_PATTERN,
                  test_pattern=SPALL_TEST_PATTERN, split_date="2020-01-01",
                  test_end=None, train_years=8, tag=None, **kwargs):
    """Train once on deduplicated SP400/500/600 data, then test on SPall.

    The train panel is restricted to dates before ``split_date`` (plus the
    horizon embargo in the embedded runner), while predictions are restricted
    to dates on or after ``split_date``. Set ``train_years`` to the desired
    look-back window; the default is 8 years. ``None`` explicitly requests all
    available history before the split.
    """
    data_dir = Path(data_dir) if data_dir else Path.cwd()
    resolved = resolve_spall_inputs(data_dir, train_pattern, test_pattern)
    split = pd.Timestamp(split_date)
    if pd.isna(split):
        raise ValueError(f"invalid split_date: {split_date!r}")
    tag = tag or f"i5_sp400_sp500_sp600_to_spall_{split.year}plus"
    ...
    model_train_source_files = summary.get("train_source_files", [])
    model_test_source_files = summary.get("test_source_files", [])
    summary.update(
        {
            "split_date": str(split.date()),
            "train_rule": f"date < {split.date()} (subject to horizon embargo)",
            "test_rule": f"date >= {split.date()}",
            "resolved_train_files": [str(path) for path in resolved["train_files"]],
            "resolved_test_files": [str(path) for path in resolved["test_files"]],
            "model_train_input_files": model_train_source_files,
            "model_test_input_files": model_test_source_files,
            "train_source_files": [str(path) for path in resolved["train_files"]],
            "test_source_files": [str(path) for path in resolved["test_files"]],
        }
    )
    if "source_duplicate_rows_dropped" in summary:
        summary.update(
            {
                "train_rows_before_dedup": summary["source_rows_before_dedup"],
                "train_rows_after_dedup": summary["source_rows_after_dedup"],
                "train_duplicate_rows_dropped": summary["source_duplicate_rows_dropped"],
            }
        )
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description=(
            "Train one I5 CNN on deduplicated SP400/SP500/SP600 data before "
            "the split date and test it on deduplicated SPall data afterward."
        )
    )
    ap.add_argument("--data-dir", default=None)
    ap.add_argument("--train-pattern", default=SPALL_TRAIN_PATTERN)
    ap.add_argument("--test-pattern", default=SPALL_TEST_PATTERN)
    ap.add_argument("--split-date", default="2020-01-01")
    ...
    ap.add_argument("--train-years", type=int, default=8,
                    help="years of training history immediately before split_date")
    ap.add_argument("--test-end", default=None)
    ap.add_argument("--seeds", type=int, default=5)
    ap.add_argument("--epochs", type=int, default=50)
